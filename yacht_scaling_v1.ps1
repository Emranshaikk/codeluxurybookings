...
$templatePath = "_template_yacht_master.html"
$siloPath = "global-yacht-silo.html"
$template = Get-Content $templatePath -Raw
$silo = Get-Content $siloPath -Raw

foreach ($kw in $keywords) {
    # Generate Slug
    $slug = $kw -replace '[^a-zA-Z0-9]', '-' -replace '-+', '-'
    ...
    # Process Template
    $titleKW = (Get-Culture).TextInfo.ToTitleCase($kw)
    $page = $template.Replace("{{KEYWORD}}", $titleKW)
    $page = $page.Replace("{{SLUG}}", $slug)
    $page = $page.Replace("<!-- GLOBAL_YACHT_SILO_CONTENT -->", $silo)
    
    Set-Content $fileName $page -NoNewline
}

Write-Host "Yacht Network Scaling Complete: $($keywords.Count) pages generated."
