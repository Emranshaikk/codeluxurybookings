...
def integrate_bottom_yacht_banner():
    count = 0
    # New Horizontal Leaderboard Widget
    BOTTOM_WIDGET_HTML = """
            <div class="maritime-partner-footer-banner" style="margin: 4rem auto 2rem; max-width: 800px; text-align: center; border-top: 1px solid rgba(212,175,55,0.1); padding-top: 3rem;">
                <p style="color: var(--primary-gold); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 3px; margin-bottom: 2rem; font-weight: 600;">Global Maritime Network Advisor</p>
...
"""
    ...
    for root, dirs, files in os.walk("."):
        for file in files:
            if file == "index.html":
                ...
                # A. Purge any legacy bottom widgets
                ...
                # B. Inject the New Leaderboard above Footer
                if "<!-- ELB_FOOTER_START -->" in content:
                    # Wipe any existing skyscraper/sidebar banners
                    ...
                    # Inject at bottom
                    content = content.replace("<!-- ELB_FOOTER_START -->", f"{BOTTOM_WIDGET_HTML}\n<!-- ELB_FOOTER_START -->")

                # C. Final Dedeplication & Sanitization
    ...
    print(f"Maritime Bottom Banner Success: {count} pages enhanced with high-performance leaderboards.")

if __name__ == "__main__":
    integrate_bottom_yacht_banner()
