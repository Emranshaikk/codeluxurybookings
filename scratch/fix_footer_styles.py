...
# Upgraded Footer HTML with 5 Services Links
...
def fix_file(path):
    ...
    # 1. Update/Inject CSS
    # Find existing MASTER FOOTER block and replace it
    css_pattern = re.compile(r'/\*\s*===\s*MASTER FOOTER \(INJECTED SITE-WIDE\)\s*===\s*\*/.*?(?=\n\s*(?:</style>|\Z))', re.DOTALL)
    if css_pattern.search(content):
        new_content = css_pattern.sub(FOOTER_CSS.strip(), content)
        if new_content != content:
            content = new_content
            modified = True
    else:
        # If it doesn't have it, inject it before </style>
        if "</style>" in content:
...
def main():
    ...
    print(f"\nSuccessfully standardized {count} HTML files!")
