...
API_ENGINE_HTML = r"""
        ...
        document.getElementById('valensEngineForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                } else { alert("Dashboard sync failed. Please use WhatsApp support."); }
    ...
    for root, dirs, files in os.walk('.'):
        for file in files:
            if file == 'index.html':
                 ...
                 # STRICT CATEGORY CHECK: MUST be Private Jet/Aviation and NOT Yacht/Villa
                 is_aviation = ('private-jet' in root or 'charter-flights' in root or 'Private Jet' in content)
                 is_yacht = 'luxury-yacht-rentals' in root or 'Yacht' in root
                 is_villa = 'luxury-villa-rentals' in root or 'Villa' in root
                 
                 # Only proceed if it is Aviation and NOT primarily something else (unless both, but generally we want to be safe)
                 if not is_aviation or (is_yacht and 'Jet' not in content) or (is_villa and 'Jet' not in content):
                     continue

                 original = content
                 
                 # 1. Cleanup all variations of the broken Valens/Aviation widgets
                 patterns = [
                     r'<!-- ELB_VALENS_API_ENGINE_START -->.*?<!-- ELB_VALENS_API_ENGINE_END -->',
                     r'<!-- VALENS_WIDGET_START -->.*?<!-- VALENS_WIDGET_END -->',
                     r'<!-- Affiliate (Aviation |Master )?Widget -->\s*<div class="affiliate-widget">.*?</div>',
                     r'<!-- Affiliate Aviation Widget -->\s*<div class="affiliate-widget">.*?</div>',
                 ...
                 # 2. Secondary check for injection if patterns didn't match perfectly
                 if 'ELB_VALENS_API_ENGINE_START' not in content:
                     if 'hero-sub' in content:
                         content = content.replace('</p>', '</p>\n' + API_ENGINE_HTML, 1)
                     elif 'blog-content' in content:
                 ...
                 if content != original:
                     with open(path, 'w', encoding='utf-8') as f:
                         f.write(content)
                     count += 1
                     print(f"API Deployed: {path}")

    print(f"\nFinal Valens API Sync complete for {count} Aviation portals.")
